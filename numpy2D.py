...
class MDSimulation:
    ...
    def __init__(self, pos, vel, r, m):
        ...
        positions and velocities in their rows as (x_i, y_i) and (vx_i, vy_i).

        r = radius of all particles
        m = mass of all particles
        n = number of particles
        pos = n x 2 array of positions of all particles [(x_i, y_i)...]
        vel = n x 2 array of velocities of all particles [(vx_i, vy_i)...]
    ...
    def advance(self, dt):
        ...
        dist = squareform(pdist(self.pos)) # Calculate the distances between positions

        # The size of a particle is measured by its diameter d = 2r. If 2 molecules come within 2r distance of each other,
        # It will be counted as a collision
        iarr, jarr = np.where(dist < 2 * self.r)

        # The way squareform works, all collisions to the left of the diagonal will also exist on the right of the diagonal
        # This line only keeps elements above / to the right of the diagonal.
        k = iarr < jarr
        ...
        for i, j in zip(iarr, jarr):
            ...
            # Why tf are you doing Matrix Multiplication?
